...
@classmethod
    def register(cls, inp_folder, overwrite):
        name = bdlTypeRegistry.register(inp_folder, overwrite)
        if name is None:
            return False
        elif name == '__duplicate__':
            _logger.info('BundleType already exists, use -o to overwrite')
            return False
        else:
            BundleDefinition.removeDefinition(name)
            definition = BundleDefinition.defForName(name)
            try:
                bType = cls.objects.get(typeName=name)
            except cls.DoesNotExist:
                bType = cls(typeName=name, typeDesc=definition.desc)
            bType.save()
        return True
